...
export class FormConstructorComponent implements OnInit, OnDestroy {
    ...
    init_gallery_images(field_name, images): void {
        this.galleryImages[field_name] = {};
        const self = this;
        if (images) {
    ...
    get_flex_padding( size: string, form_type: string, record: SitebillModelItem ): string {
        if ( record.type === 'hidden' || record.hidden === true ) {
            return '';
        }
        let css_class = 'p-12';
